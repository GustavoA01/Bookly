...
export const AddListContent = ({ id, open, setOpen }: AddListContentProps) => {
  ...
  return (
    <div className="max-h-64 overflow-y-auto">
      {isLoading && <p className="text-center py-4 text-muted-foreground">Carregando...</p>}
      ...
      {lists && lists.length > 0 ? (
        <>
          {lists?.map((list) => (
            <div
              key={list.id}
              onClick={() => addToListFn({ listId: list.id, bookId: id!, action: 'add' })}
            ...
            >
              ...
              {list.books.length > 0 ? (
                <p className="text-muted-foreground text-sm">
                  {list.books.length === 1 ? '1 livro' : `${list.books.length} livros`}
                </p>
              ) : (
                <p className="text-muted-foreground text-sm">Nenhum livro adicionado</p>
              )}
            </div>
          ))}
        </>
